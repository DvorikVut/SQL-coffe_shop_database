...
DROP PROCEDURE get_coffee_details;
...
-- trigger check (I guess rabotaet tak)
    -- DELETE FROM Zakaznik WHERE User_ID = 1;
...
CREATE OR REPLACE PROCEDURE get_coffee_details (
    coffee_blend_name IN VARCHAR2,
    coffee_details OUT SYS_REFCURSOR
) AS
BEGIN
    OPEN coffee_details FOR
        SELECT Slozeni, Popis_Chuti, Kvalita
        FROM Kava
        WHERE Slozeni = coffee_blend_name;
END;
/

-- procedure that takes the name of the coffee blend as input and returns its details
DECLARE
    coffee_cursor SYS_REFCURSOR;
    name VARCHAR2(50) := 'Arabica 100%'; -- Specify the coffee blend name here
    slozeni Kava.Slozeni%TYPE;
    popis_chuti Kava.Popis_Chuti%TYPE;
    kvalita Kava.Kvalita%TYPE;
BEGIN
    get_coffee_details(name, coffee_cursor);
    -- Fetch data from the cursor
    LOOP
        FETCH coffee_cursor INTO slozeni, popis_chuti, kvalita;
        EXIT WHEN coffee_cursor%NOTFOUND;
        -- Display or use the retrieved data as needed
        DBMS_OUTPUT.PUT_LINE('Coffee Blend: ' || slozeni || ', Description: ' || popis_chuti || ', Quality: ' || kvalita);
    END LOOP;
    CLOSE coffee_cursor;
END;
/
...
GRANT ALL ON prehled_kavaren_podle_casu_otevreni TO XDVORY00;

-------- SELECT + WITH + CASE --------
WITH AverageParticipants AS (
    SELECT
        CASE
            WHEN EXTRACT(HOUR FROM CAS) BETWEEN 6 AND 11 THEN 'Ráno'
            WHEN EXTRACT(HOUR FROM CAS) BETWEEN 12 AND 17 THEN 'Odpoledne'
            WHEN EXTRACT(HOUR FROM CAS) BETWEEN 18 AND 23 THEN 'Večer'
            END AS DayPart,
        AVG(Kapacita - Info_o_volnych_mistech) AS AvgParticipants
    FROM
        Cupping_akce
    GROUP BY
        CASE
